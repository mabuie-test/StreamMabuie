...
const limiter = rateLimit({ windowMs: RATE_WINDOW, max: RATE_MAX });
app.use('/api/', limiter);
...
app.post('/api/frame/:deviceId', express.raw({ type: ['image/jpeg','application/octet-stream'], limit: `${process.env.MAX_FRAME_MB || 3}mb` }), async (req, res) => {
  try {
    ...
    // simple protection: ignore empty or too small frames
    if (buf.length < 100) return res.status(400).send('frame too small');
